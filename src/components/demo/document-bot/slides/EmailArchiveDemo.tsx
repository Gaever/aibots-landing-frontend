...
interface EmailArchiveDemoProps {
  autoStart?: boolean;
}

export function EmailArchiveDemo({ autoStart = false }: EmailArchiveDemoProps) {
  const [messages, setMessages] = useState<Array<{ role: string; text: string }>>([]);
  const [isTyping, setIsTyping] = useState(false);
  const scenario = landingContent.documentBotPresentation.demo.email.messages;
  ...
  return (
    <TelegramFrame
      title={landingContent.documentBotPresentation.demo.email.header.title}
      subtitle={isTyping ? "печатает..." : landingContent.documentBotPresentation.demo.email.header.subtitle}
      avatar="📧"
    >
